 ...
 *
 * Renders any project from data/projects.json. Individual project pages set
 * $project_slug and require this file — all markup, fallback/placeholder
 * behavior, and template-level copy lives here so no project page
...
/* Uses assets/images/project-placeholder.jpg once that file exists;
   falls back to this image for any project without its own photography. */
$project_placeholder_path = 'assets/images/project-placeholder.jpg';
$fallback_image = is_file(dirname(__DIR__) . '/' . $project_placeholder_path)
    ? $project_placeholder_path
    : 'assets/images/hero-villa-pool.jpg';
...
/* ---------- Starting price (one shared source for hero card, sticky CTA,
   and the JSON-LD offer below — see project_price_info()) ---------- */
$price_info = project_price_info($project);

/* ---------- Sticky CTA (action is fixed template copy) ---------- */
$sticky_label = $price_info['label'];
$sticky_value = $price_info['value'];
$sticky_action = 'Fill The Form';

/* ---------- Breadcrumb trail: Home > Developer > Project when the project's
   developer has a matching archive page, otherwise Home > Communities > Project ---------- */
$breadcrumb_developer = get_developer_by_canonical_name($project['developer']);
$breadcrumb_trail = $breadcrumb_developer !== null
    ? [
        ['label' => $breadcrumb_developer['display_name'], 'url' => $breadcrumb_developer['slug'] . '.php'],
        ['label' => $project['name']],
    ]
    : [
        ['label' => 'Communities', 'url' => 'communities.php'],
        ['label' => $project['name']],
    ];
...
if ($price_info['available'] && $price_info['numeric_aed'] !== null) {
    $json_ld['offers'] = [
        '@type' => 'Offer',
        'priceCurrency' => 'AED',
        'price' => (string) $price_info['numeric_aed'],
        'availability' => 'https://schema.org/InStock',
    ];
}

/* ---------- Enquiry-form source-attribution context (shared partial) ---------- */
$enquiry_form_context = [
    'page_title' => $page_title,
    'page_slug' => pathinfo($current_file, PATHINFO_FILENAME),
...
<?php endif; ?>
        ...
        <div class="hero__card" id="hero-form-card">
          <p class="price-label"><?= smb_e($price_info['label']) ?></p>
          <p class="price"><?= smb_e($price_info['value']) ?><?php if ($price_info['available']): ?><span class="price__note">*</span><?php endif; ?></p>
<?php if ($price_info['available']): ?>
...
<?php endif; ?>
<?php
          $enquiry_form_variant = 'hero';
          $enquiry_form_instance = 'hero';
          $enquiry_form_field_prefix = 'hf';
          $enquiry_form_privacy_note = 'Our Team Will Contact You Shortly.';
          require __DIR__ . '/../template-parts/enquiry-form.php';
?>
        </div>
      </div>
    </section>
...
?>
          <figure class="gallery__item<?= $is_featured ? ' gallery__item--featured' : '' ?>" data-reveal>
            <button type="button" class="gallery__item-trigger" aria-label="<?= smb_e('View Larger: ' . $alt) ?>">
              <img src="<?= smb_e($src) ?>"
                   alt="<?= smb_e($alt) ?>"
...
<?php endforeach; ?>
    ...
    <div class="lightbox" id="gallery-lightbox" hidden>
      <div class="lightbox__dialog" role="dialog" aria-modal="true" aria-label="Project Gallery Viewer">
        ...
        <div class="lightbox__zoom">
          <button type="button" class="lightbox__zoom-out" aria-label="Zoom Out">
            <svg class="icon" aria-hidden="true"><use href="#i-zoom-out"/></svg>
          </button>
          <button type="button" class="lightbox__zoom-in" aria-label="Zoom In">
            <svg class="icon" aria-hidden="true"><use href="#i-zoom-in"/></svg>
          </button>
        </div>
      </div>
    </div>
...
<?php
          $enquiry_form_variant = 'card';
          $enquiry_form_instance = 'main';
          $enquiry_form_field_prefix = 'mf';
          require __DIR__ . '/../template-parts/enquiry-form.php';
?>
        </div>
      </div>
